...
SNAKE_FOOD = pygame.Rect(320, 260, CELL_SIZE, CELL_SIZE)
# Define button areas
button_width = 240
button_height = 100
button_margin = 50
...
while running:
    ...
    if not game_over:
        ...
        # background sip
            pygame.mixer.music.load('Hiss sound effect.mp3')
            pygame.mixer.music.play(start=4.0, fade_ms=-1000)
            SNAKE_FOOD = pygame.Rect(random.randint(0, SCREEN_WIDTH - CELL_SIZE) // CELL_SIZE * CELL_SIZE, random.randint(0, SCREEN_HEIGHT - 200 - CELL_SIZE) // CELL_SIZE * CELL_SIZE, CELL_SIZE, CELL_SIZE)
        else:
            SNAKE_BODY.pop(0)
            
        # Snake to Body & Snake to Boundary collision
        if (new_head[0] < 0 or new_head[0] >= SCREEN_WIDTH or new_head[1] < 0 or new_head[1] >= SCREEN_HEIGHT - 200):
            game_over = True
    ...
    if game_over:
        ...
        pygame.draw.rect(screen, FOOD_COLOR, SNAKE_FOOD, 0)
